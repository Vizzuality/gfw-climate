  ...
  function(
    ...
    var LineChartIndicator = IndicatorView.extend({
      ...
      fetchIndicator: function(data) {
        ...
        if (this.model.get('location_compare')) {
          // set data compare
          var dataCompare = _.extend({}, data, {
            location: this.model.get('location_compare')
          });

          $.when(
            this.model.fetch({ data: this.setFetchParams(data) }),
            this.modelCompare.fetch({ data: this.setFetchParams(dataCompare) })
          ).done(
            function() {
              this.render();
          ...
          // Fetch values
          this.model.fetch({ data: this.setFetchParams(data) }).done(
            function() {
              this.render();
              this.$el.removeClass('is-loading');
            }.bind(this)
      ...
      _drawGraph: function() {
        ...
        if (!!data.length) {
          // Set range
          var arr = this.model.get('location_compare')
            ? _.union(this.model.get('data'), this.modelCompare.get('data'))
            : this.model.get('data');
          var range = [
            0,
            _.max(arr, function(o) {
              return o.value;
            }).value
          ];
