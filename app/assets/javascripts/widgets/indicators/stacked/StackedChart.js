  ...
  function($, d3, _, Handlebars, mps, tooltipTpl) {
    ...
    StackedChart.prototype._drawData = function() {
      ...
      this.bars = this.svg
        ...
        .style('fill', function(d, i) {
          return self._getColorByIndex(i);
        });
      ...
      this.text = this.bars
        ...
        .attr('x', function(d) {
          return getXPos(d) + self.x.rangeBand() / 4;
        })
        .attr('text-anchor', 'middle')
        .style('fill', '#000')
        .text(function(d, i) {
          return self.stackedTotals[i];
        });
    };
    ...
    StackedChart.prototype._drawTooltip = function() {
      ...
      this.rectangles
        ...
        .on('mousemove', function(d) {
          ...
          mps.publish(
            'StackedChart/mousemove' +
              self.options.slug_compare +
              self.options.id,
            [d[0]]
          );
        })
    ...
    StackedChart.prototype.setListeners = function() {
      ...
      if (self.lock) {
        this.subcriptions = [
          mps.subscribe(
            'StackedChart/mouseout' + this.options.slug + this.options.id,
            function() {
              if (!!self.svg) {
                self.tooltip
