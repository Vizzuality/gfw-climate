...
define(
  ...
  function(CartoDBLayerClass, plantationsCartocss) {
    ...
    var PlantationsLayerBySpecies = CartoDBLayerClass.extend({
      options: {
        sql:
          "SELECT the_geom_webmercator, cartodb_id, type_text, spec_org, spec_simp, area_ha, percent, '{tableName}' AS tablename, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
