...
define(['abstract/layer/CartoDBLayerClass'], function(CartoDBLayerClass) {
  ...
  var NamResourceRightsLayer = CartoDBLayerClass.extend({
    options: {
      sql:
        "SELECT 'nam_resource_rights' as tablename, cartodb_id, the_geom_webmercator, name, round(area_ha::float) as area_ha,  '{tableName}' as layer, {analysis} AS analysis FROM {tableName}",
      infowindow: true,
