...
define(['abstract/layer/CartoDBLayerClass'], function(CartoDBLayerClass) {
  ...
  var IdnLeuserLayer = CartoDBLayerClass.extend({
    options: {
      sql:
        "SELECT 'idn_leuser' as tablename, cartodb_id, the_geom_webmercator, name,  round(area_ha::float) as area_ha, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
      infowindow: true,
