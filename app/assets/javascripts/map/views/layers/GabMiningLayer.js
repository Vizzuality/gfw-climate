...
define(['abstract/layer/CartoDBLayerClass'], function(CartoDBLayerClass) {
  ...
  var GabMiningLayer = CartoDBLayerClass.extend({
    options: {
      sql:
        "SELECT 'gab_mining' as tablename, cartodb_id, the_geom_webmercator, titulaire as company, superfic_1 as area_ha, pa_num as name, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
      infowindow: true,
