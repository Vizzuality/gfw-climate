...
define(
  ...
  function(CartoDBLayerClass, concesiones_forestalesCartoCSS) {
    ...
    var ConcesionesForestalesNotSupervised = CartoDBLayerClass.extend({
      options: {
        sql:
          "SELECT the_geom_webmercator, title_holder, area_ha, type, province, contract, department, supervision, '{tableName}' AS layer, {analysis} AS analysis, '{tableName}' AS name FROM {tableName}",
        infowindow: true,
        interactivity:
          'title_holder, area_ha, type, province, department, contract, supervision, analysis',
