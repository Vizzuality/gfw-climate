...
define(['abstract/layer/CartoDBLayerClass'], function(CartoDBLayerClass) {
  ...
  var GabWoodFiberPlantationsLayer = CartoDBLayerClass.extend({
    options: {
      sql:
        "SELECT 'gab_wood_fiber' as tablename, cartodb_id, the_geom_webmercator, nom_planta as name, operateur as company, sup_sig as area_ha, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
      infowindow: true,
