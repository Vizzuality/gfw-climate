...
define(['abstract/layer/CartoDBLayerClass'], function(CartoDBLayerClass) {
  ...
  var CanLoggingLayer = CartoDBLayerClass.extend({
    options: {
      sql:
        "SELECT 'can_logging' as tablename, cartodb_id, the_geom_webmercator, company, country, round(area_ha::float) as area_ha, name, source, type, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
      infowindow: true,
