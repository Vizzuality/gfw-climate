...
define(
  ...
  function(CartoDBLayerClass, plantationsCartocss) {
    ...
    var PlantationsLayerByType = CartoDBLayerClass.extend({
      options: {
        sql:
          "SELECT the_geom_webmercator, cartodb_id, type_text, spec_org, spec_simp, percent, area_ha, '{tableName}' AS tablename, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
