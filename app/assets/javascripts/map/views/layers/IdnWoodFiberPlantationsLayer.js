...
define(['abstract/layer/CartoDBLayerClass'], function(CartoDBLayerClass) {
  ...
  var IdnWoodFiberPlantationsLayer = CartoDBLayerClass.extend({
    options: {
      sql:
        "SELECT 'idn_wood_fiber' as tablename, cartodb_id, the_geom_webmercator, name, group_comp as company, area_ha, type, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
      infowindow: true,
      interactivity:
