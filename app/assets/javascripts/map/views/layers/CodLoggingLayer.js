...
define(['abstract/layer/CartoDBLayerClass'], function(CartoDBLayerClass) {
  ...
  var CodLoggingLayer = CartoDBLayerClass.extend({
    options: {
      sql:
        "SELECT 'cod_logging' as tablename, cartodb_id, the_geom_webmercator, attributai as company, round(sup_adm_ha::float) as area_ha, num_ga as name, '{tableName}' AS layer, {analysis} AS analysis FROM {tableName}",
      infowindow: true,
