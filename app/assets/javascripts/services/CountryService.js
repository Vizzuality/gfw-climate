define(
  ...
  function(Class, UriTemplate, _, Promise, ds) {
    ...
    var GET_REQUEST_COUNTRY_STATS_ID = 'CountryService:getCountryStats';
    var GET_REQUEST_COUNTRY_ID = 'CountryService:getCountry';
    var GET_REQUEST_REGIONS_LIST_ID = 'CountryService:getRegionsList';
    var SHOW_REQUEST_REGION_ID = 'CountryService:showRegion';
    ...
    var APIURLS = {
      ...
      getCountriesListGeo:
        '/query?sql=SELECT name_engli as name, iso, topojson FROM {countriesDataset} WHERE iso IN({climateCountries}) ORDER BY name ASC',
      getCountriesStats:
        "/query/?sql=SELECT * FROM {countryStatsDataset} WHERE iso='{iso}'",
      getCountry:
        "/query?sql=SELECT name_engli as name, iso, topojson FROM {countriesDataset} WHERE iso='{iso}'",
      getRegionsList:
        "/query?sql=SELECT cartodb_id, iso, bbox as bounds, id_1, name_1 FROM {regionsDataset} WHERE iso='{iso}' ORDER BY name_1",
      showRegion:
        "/query?sql=SELECT id_1, name_1, geojson FROM {regionsDataset} WHERE iso='{iso}' AND id_1={region} ORDER BY name_1",
      getSubRegionsList:
        "/query?sql=SELECT id_1, name_1 FROM {subRegionsDataset} WHERE iso='{iso}' ORDER BY name_1"
    };
    ...
    var CountriesService = Class.extend({
      ...
      getClimateConfig: function(params) {
        return new Promise(
          function(resolve, reject) {
            ...
            this.defineRequest(GET_REQUEST_COUNTRY_CONFIG_ID, url, {
              type: 'persist',
              duration: 1,
              unit: 'days'
            });
            ...
            var requestConfig = {
              ...
              success: function(res, stat) {
                if (res.data && res.data.length > 0) {
                  var climateCountries =
                    res.data &&
                    res.data.map(function(country) {
                      return country.iso;
                    });
                  resolve(climateCountries, stat);
                } else {
            ...
            this.getClimateConfig()
              .then(
                function(countryConfig) {
                  var params = {
                    climateCountries: "'" + countryConfig.join("','") + "'"
                  };
                  var status = _.extend({}, CONFIG, params);
                  var urlTemplate = filters.geo
                    ? APIURLS.getCountriesListGeo
                    : APIURLS.getCountriesList;
                  var templateId = filters.geo
                    ? GET_REQUEST_COUNTRIES_LIST_ID + '_GEO'
                    : GET_REQUEST_COUNTRIES_LIST_ID;
              ...
              .then(
                function() {
                  var url = new UriTemplate(
                    APIURLS.getCountriesStats
                  ).fillFromObject(params);

                  this.defineRequest(GET_REQUEST_COUNTRY_STATS_ID, url, {
                    type: 'persist',
                    duration: 1,
                    unit: 'days'
                  });
                ...
                .then(
                  function(countryStats) {
                    ...
                    var requestConfig = {
                      ...
                      success: function(res, stat) {
                        var dataCountryStats =
                          countryStats.length >= 0 ? countryStats[0] : [];
                        var dataCountry =
      ...
      showRegion: function(params) {
        ...
        return new Promise(
          function(resolve, reject) {
            var status = _.extend({}, CONFIG, params);
            var url = new UriTemplate(APIURLS.showRegion).fillFromObject(
              status
            );
