    ...
    var TimelineBtnClass = Backbone.View.extend({
      ...
      initialize: function(layer, currentDate) {
        ...
        if (currentDate && currentDate[0]) {
          ...
          this._updateCurrentDate([
            this.data[this.data.length - 1].start,
      ...
      fillSelects: function() {
        ...
        _.each(this.data, function(label, i) {
          ...
          if (
            label.start.format(dformat) == currentDate[0].format(dformat) &&
            label.end.format(dformat) == currentDate[1].format(dformat)
          ) {
            currentIndex = i;
          }
        });
      ...
      render: function(callback) {
        ...
        this.xscale = d3.scale
          .linear()
          .domain([
            moment(this.options.dateRange[0]).month(),
            moment(this.options.dateRange[1]).month()
          ])
          .range([0, width])
          .clamp(true);
        ...
        this.tickG = this.svg
          ...
          .attr(
            ...
            _.bind(function(d, i) {
              ...
              var x =
                i * slotWidth +
                (slotWidth - this.options.tickWidth) / 2 -
                slotWidth;
        ...
        this.tickG.on('click', function(date, i) {
        ...
        if (!el) {
          el = this.tickG
            .filter(function(d) {
              var dformat = 'DD-MM-YYYY';
              return (
                d.start.format(dformat) === date.start.format(dformat) &&
