    ...
    var CountryWidgetsModalView = ModalView.extend({
      ...
      parseData: function() {
        ...
        if (widgets[forestAndCarbon]) {
          ...
          for (
            var i = 0, wLength = widgets[forestAndCarbon].length;
            i < wLength;
            i++
          ) {
            if (widgets[forestAndCarbon][i].slug === peatDrainageSlug) {
              widgets[forestAndCarbon][i].hide =
                peatDrainageCountries.indexOf(country) < 0;
            }
          }
        }
        ...
        return {
          view: {
            ...
            isSubNational:
              this.status.get('view') == 'subnational' ? true : false,
      ...
      cacheVars: function() {
        ...
        this.$jurisdictionsBoxes = this.$el.find(
          '.js-field-list-checkbox-jurisdiction'
        );
      ...
      setJurisdictionStatus: function() {
        _.each(
          this.$jurisdictionsBoxes,
          _.bind(function(el) {
            var is_active = _.contains(
              this.status.get('jurisdictionsIds'),
              $(el).data('id')
            );
            $(el).toggleClass('is-active', is_active);
          }, this)
