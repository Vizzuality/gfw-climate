define(
  ...
  function(Handlebars, Backbone, _, d3, NumbersHelper, tpl) {
    'use strict';
    ...
    var PieChart = Backbone.View.extend({
      ...
      _parseData: function() {
        ...
        this.data.top_five.forEach(
          function(data, index) {
            data.color = this.defaults.buckets[index];
            data.name = data.province;
            data.displayValue = NumbersHelper.addNumberDecimals(
        ...
        others.name = 'Others';
        others.displayValue = NumbersHelper.addNumberDecimals(
          Math.round(others.value)
        );
      ...
      _setUpGraph: function() {
        ...
        this.svg = svg
          ...
          .attr(
            'transform',
            'translate(' +
              (this.cWidth + margin.left + margin.right) / 2 +
              ',' +
