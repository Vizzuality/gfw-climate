  ...
  function(Backbone, mps, PresenterClass, CountryModel, WidgetCollection) {
    ...
    var CountryModalWidgetsPresenter = PresenterClass.extend({
              ...
              _.bind(function(response) {
                ...
                if (!!p.options) {
                  ...
                  if (!!p.options.jurisdictions) {
                    jurisdictionsActive = _.map(
                      p.options.jurisdictions,
                      function(j) {
                        return ~~j.idNumber;
                      }
                    );
                  ...
                  var activeWidgets = _.where(this.status.get('widgets'), {
                    default: true
                  });
      ...
      changeActiveWidgets: function(widgetId, remove) {
        var widgets = _.clone(
          this._toInteger(this.status.get('widgetsActive'))
        );
        var view = this.status.get('view');
        remove
          ? (widgets = _.without(widgets, widgetId))
      ...
      changeActiveJurisdictions: function(jurisdictionId, remove) {
        ...
        remove
          ? (jurisdictions = _.without(jurisdictions, jurisdictionId))
      ...
      checkSelectionReached: function() {
        if (
          this.status.get('widgetsActive').length +
            this.status.get('jurisdictionsIds').length +
            this.status.get('areasIds').length >
          this.maxSelection
        ) {
